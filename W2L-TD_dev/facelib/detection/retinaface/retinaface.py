...
class RetinaFace():
    ...
    def __init__(self, ort_sess, network_name='resnet50', half=False, phase='test'):
        ...
        self.mean_tensor = np.array([[[[104]], [[117]], [[123]]]],dtype=np.float32)
    ...
    def __detect_faces(self, inputs):
        ...
        self.scale = np.array([width, height, width, height], dtype=np.float16)
        tmp = [width, height, width, height, width, height, width, height, width, height]
        # self.scale1 = torch.tensor(tmp, dtype=torch.float32).to(device)
        self.scale1 = np.array(tmp, dtype=np.float16)
        ...
        loc, conf, landmarks = self.ort_sess.run(None, {'img': inputs})
    ...
    def batched_detect_faces(self, frames, conf_threshold=0.8, nms_threshold=0.4, use_origin_size=True):
        ...
        b_loc_and_conf = np.concatenate((b_loc, np.expand_dims(b_conf, -1)), dim=2).float()
    ...
    def batched_detect_faces_bbox(self, frames, conf_threshold=0.8, nms_threshold=0.4, use_origin_size=True):
        ...
        # concat
        # b_loc_and_conf = torch.cat((b_loc, b_conf.unsqueeze(-1)), dim=2).float()
        b_loc_and_conf = np.concatenate((b_loc, np.expand_dims(b_conf, -1)), axis=2)
